...
function buttonToggles() {
  ...
  let buttons = ['left', 'right', 'middle'].map((button, i) => {
    ...
    function onClick() {
      socket.emit('button-toggle', button, active ? 'up' : 'down');
      keyDispatcher.dispatch({ type: 'button-toggle', button: button});
    }
  ...
  let keys = ['control', 'shift', 'alt', 'meta'].map((key, i) => {
    ...
    function onClick() {
      socket.emit('key-toggle', key, active ? 'up' : 'down');
      keyDispatcher.dispatch({ type: 'key-toggle', key: key});
    }
    return (<button className={className} key={i} onClick={onClick}>
      {key}
    </button>);
